...
class JsonToCsvConverter:
    def __init__(self):
        self.parsed_data_dir = Path(".")  # Current directory (csvOct6)
        self.output_dir = Path("csvData")  # Output in csvOct6/csvData
    ...
    def get_elected_person_votes_from_station(self, station_data: Dict, elected_person_index: int) -> int:
        ...
        if elected_person_index < len(candidate_votes):
            vote_count = candidate_votes[elected_person_index]
    ...
    def calculate_station_evm_votes(self, station_data: Dict) -> int:
        ...
        valid_votes = [v for v in candidate_votes if v is not None]
        return sum(valid_votes)
    ...
    def convert_json_to_csv(self, json_file_path: Path) -> bool:
        ...
        try:
            ...
            if not serial_wise_details:
                ...
                for station in serial_wise_details:
                    # Calculate station-specific values (handle None values)
                    station_valid_votes = station.get("Total Number of valid votes") or 0
                    station_rejected_votes = station.get("Number of Rejected votes") or 0
                    station_nota = station.get("NOTA") or 0
                    station_total = station.get("Total") or 0
                    station_tendered_votes = station.get("Number Of Tender Votes") or 0
